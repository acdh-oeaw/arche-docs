...
* `{children depth}` and `{parents depth}` specify how far from the **_requested resource_** in the repository structure we want go (by following the `{repoCfg}$.schema.parent` RDF property)
* `{pointed to}` and `{pointed from}` are binary flags (they take only values od `0` or `1`) and indicate if 
...
* RDF property names passed trough these parameters have to be fully-qualified URIs. There's no option to use prefix-shortened names, e.g. you have to specify the `https://vocabs.acdh.oeaw.ac.at/schema#hasTitle` and `acdh:hasTitle` wont' work.
