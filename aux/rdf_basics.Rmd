...
A solution to this problem is called *framing* and is discussed in a separate chapter.

## Object IDs being URIs

You may face similar issue with the RDF graph nodes - mapped to objects - identifiers:

* If you are attached to integer ids, sorry, it's not the case.
* You should pay attention to blank nodes as if their identifiers can be compared only within a single graph (and if they match an id of a blank node from another graph, then, surprisingly, they aren't considered equal).

## Literal objects immutability

Another complexity is brought by literals.

Literals have compound structure with value, datatype and language tag which requires mapping them to objects. The important thing is these objects should be immutable.

Let's consider a following RDF graph:

```
Resource1(someUri) --someProperty--> Literal1(someValue, someDatatype, someLang)
                                        ^
Resource2(otherUri) --someProperty------|
```

mapped to two objects `resource1` and `resource2` sharing a reference to a common `literal1` object.
...
By the way means the `resource1->someProperty->setValue(NEWVALUE)` syntax shouldn't exist and the right one should be `resource1.someProperty = resource1.someProperty.withValue(NEWVALUE)` or during the parsing of the RDF graph into objects of your programming language a separate `literal1` objects should be created for `resource1.someProperty` value and `resource2.someProperty` value.
...
What you should remember is that **a given RDF data set may have virtually countless valid serializations even in a single serialization formats.**
...
* n-triples - lines below in any order
  ...
  ```
* turtle
  * all n-triples serialization variant (n-triples is a subset of turtle)
  * ```
    @prefix foo: <http://foo/> .
  ...
  * ```
   ...
   _:b1            <http://baz> "baz" .
    ```
  * and countless others as we can freely choose which namespace to declare and which aliases assign to them
* JSON-LD:
