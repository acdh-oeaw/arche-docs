...
* Ordering by properties of linked resources.  
  Let's say you want to order by, let's say a title of a parent resource and then by a resource title.  
  This is (at least currently) impossible.
...
* For the exact description on how the search phrase is parsed please refer to the
...
First, it's crucial to understand that the full text search match highlighting works fully independently from the actual search.  
**The actual search works in the 1st step of the [search workflow](#search-workflow) while the FTS matches highlighting works in the 4th step.**

The main reason for that is performance - FTS matches highlighting is a very costly operation (especially on large content like resource's binary payloads)
and it only makes sense to apply it to data actually being returned, after the paging being applied.

* Technically it's possible (although it doesn't make much sense) to perform a full text search against one property and match highlighting against another.

TODO:
* simple example
* many triples with the highlight
* no way to determine the property giving the match if `ftsProperty` isn't specified (what happens then actually?!)
